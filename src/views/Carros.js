...
import CarrosC from '../components/Card';
...
export default function Carros ({route,navigation}){
    ...
    async function carregarCarros() {
        let reqs = await fetch(config.urlRootPhp+'ListarCarros.php',{
            method: 'POST',
            headers:{
    ...
    _renderItem = ({item,index})=>{
        return (
            <TouchableOpacity /*onPress={() => navigation.navigate('CadFunc',{empresaId:item.id})}*/>
                <CarrosC modelo= {item.modelo} velocidade = {item.velocidade_max} status={item.status}/>
            </TouchableOpacity>
        )
    }
               ...
               <View>
                    <View>
                            <TouchableOpacity style = {style.button}>
                                <Text style = {style.buttonText} onPress={() => navigation.navigate('Funcionarios',{empresaId:empresaId})}>Funcionarios</Text>
                            </TouchableOpacity>
                    </View>

                    <View>
                            <TouchableOpacity style = {style.button2}>
                                <Text style = {style.buttonText2}>Carros</Text>
                            </TouchableOpacity>
                    </View>
               </View>

               <View style = {style.view1}>
                    <TouchableOpacity style = {style.button} onPress={() => navigation.navigate('CadCarro',{empresaId:empresaId})}>
                        <Text style = {style.buttonText}>Cadastrar Carros</Text>
                    </TouchableOpacity>
               </View>
